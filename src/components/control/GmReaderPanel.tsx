...
import {
  getGmScriptSceneById,
  getGmScriptSceneIndex,
  mission01GmScript,
  type GmScriptScene
} from '../../data/mission01GmScript';
...
export default function GmReaderPanel({ activeSceneId, onSelectScene, className = '' }: GmReaderPanelProps) {
  ...
  const scene: GmScriptScene = getGmScriptSceneById(activeSceneId);
  const sceneIndex = getGmScriptSceneIndex(activeSceneId);
  const previousScene = sceneIndex > 0 ? mission01GmScript[sceneIndex - 1] : null;
  const nextScene = sceneIndex < mission01GmScript.length - 1 ? mission01GmScript[sceneIndex + 1] : null;
      ...
      <div className="border-b border-stone-800/90 bg-stone-900/70 px-4 py-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="shrink-0 rounded border border-emerald-700/50 bg-emerald-950/20 px-2 py-1 text-[10px] font-mono font-bold uppercase tracking-widest text-emerald-300">
            SCÈNE {sceneIndex + 1} / {mission01GmScript.length}
          </div>
        </div>
